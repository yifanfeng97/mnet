...
def split_meshes(cfg):
    train_meshes = get_mesh_file_names('train', cfg.data_simply_mesh_root)
    test_meshes = get_mesh_file_names('test', cfg.data_simply_mesh_root)
...
def split_pc(cfg):
    train_meshes = get_pc_file_names('train', cfg.data_pc_root)
    test_meshes = get_pc_file_names('test', cfg.data_pc_root)
...
def get_mesh_file_names(state, root):
    file_names = []
...
def get_pc_file_names(state, root):
    file_names = []
    class_dirs = glob.glob(osp.join(root, '*'))
    class_dirs = sorted(class_dirs)
    class_dirs = [class_dir for class_dir in class_dirs if osp.isdir(class_dir)]
    for label, class_dir in enumerate(class_dirs):
        ...
        meshes = glob.glob(osp.join(class_dir, state, '*.npy'))
        meshes = sorted(meshes)
        for mesh in meshes:
            shape_name = osp.split(mesh)[1].split('.')[0]
            file_names.append({'label': label,
